			...
			handler : function () {
				if ($("#empProgressEditForm").form("validate")) {
					...
					if(!$("#empProgressEditForm").form("validate")) {
						...
						$.ajax({
							url: "EmpProgress/EmpProgress_ajaxCheckEmpDepart.action",
							data: {
								"empProgress.studentObj.studentNo": global_studentNo //$("#empProgress_studentObj_studentNo_edit").val()
							},
							success: function(data,response,status) {
								if(!data.success) {
									alert("该学生不是你所在班级的学生不能评价！");
								} else { 
						...
						if (empProgress) { 
							...
							$("#empProgress_studentObj_studentNo_edit").combobox({
						        ...
						        onLoadSuccess: function () { //数据加载完毕事件
						            ...
						            //}
									global_studentNo = empProgress.studentObjPri;
								}
							});
							$("#empProgress_addTime_edit").val(empProgress.addTime);
							$("#empProgress_evaluate_edit").val(empProgress.evaluate);
							$("#empProgress_evaluate_edit").validatebox({
								required : true,
								missingMessage : "请选择评价",
								editable: false
							});
